                    ...
                    SubCommand::with_name("deploy")
                        ...
                        .arg(
                            Arg::with_name("seed")
                                ...
                                .required(false)
                                .help(
                                    "The seed of the program pubkey to be deployed \
                                    [default: None]",
                                ),
                        )
                        .arg(
                            Arg::with_name("seed-id")
                                .long("seed-id")
                                .value_name("seed-id")
                                .takes_value(true)
                                .required(false)
                                .help(
                                    "The pubkey of the program to be deployed \
                                    [default: None]",
                                ),
                        )
                        .arg(
...
pub fn parse_program_subcommand(
    ...
    let response = match (subcommand, sub_matches) {
        ("deploy", Some(matches)) => {
            ...
            let seed = value_of(matches, "seed");
            let seed_id = value_of(matches, "seed-id");
            CliCommandInfo {
...
fn process_program_deploy(
    ...
    let result = if do_initial_deploy {
        ...
        do_process_program_deploy(
            ...
            fee_payer_signer,
            &[upgrade_authority_signer, upgrade_authority_signer],
            buffer_signer,
            &buffer_pubkey,
            buffer_program_data,
            upgrade_authority_signer,
...
pub fn deploy_with_max_program_len(
    ...
    let seed_pubkey = if let Some(seed_id) = seed_id {
        Pubkey::from_str(seed_id).unwrap()
    } else {
        *program_address // Use program_address as default when no seed_id
    };
    
    let programdata_address = get_program_data_address(&seed_pubkey);
    
    if let Some(seed) = seed {
        println!("create_account_with_seed called with seed: {}", seed);
        Ok(vec![
            system_instruction::create_account_with_seed(
                payer_address,
                &seed_pubkey,
                upgrade_authority_address, // PATCH: This lets us deploy with seed accounts
                seed,
                program_lamports,
